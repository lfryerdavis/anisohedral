...
The `-depth` option specifies how deep in the recursion you wish to go. There were a total of 3,914,597 shapes generated by this command. To assist with debugging, there is a `-debug` option, where 0 (the default) gives no output, and 3 gives a lot of output. Each file in the code has its own set of debug variables to control the amount of debug output while programming. 
